...
public class AssetsOrdersServiceImpl implements IAssetsOrdersService {
    ...
    public int insertAssetsOrders(List<AssetsOrders> assetsOrdersList, SysDept sysDept) {
        for (AssetsOrders assetsOrders : assetsOrdersList) {
            ...
            assetsOrders.setState(0L);
            assetsOrders.setRecordId(IdUtils.getOrderIdByUUId());
            assetsOrders.setCreateTime(DateUtils.getNowDate());
            assetsOrders.setSenderId(sysDept.getDeptId());
            assetsOrders.setSenderName(sysDept.getDeptName());
    ...
    public int confirmOrders(List<Long> assetsOrdersIds, SysDept sysDept) {
        for (Long id : assetsOrdersIds) {
            ...
            assetsOrders.setCreateTime(DateUtils.getNowDate());
            assetsItemRecordService.insertAssetsItemRecordOperation(assetsOrders, sysDept);
            AssetsSchoolWarehouse assetsSchoolWarehouse = assetsSchoolWarehouseMapper.selectAssetsSchoolWarehouseByItemIdAndSid(assetsOrders.getItemId(), sysDept.getParentId());


            if (assetsSchoolWarehouse == null) {
                assetsSchoolWarehouse = new AssetsSchoolWarehouse();
                ...
                assetsSchoolWarehouse.setCreateTime(DateUtils.getNowDate());
                assetsSchoolWarehouse.setSchoolId(sysDept.getParentId());
                assetsSchoolWarehouseMapper.insertAssetsSchoolWarehouse(assetsSchoolWarehouse);
            } else {
                assetsSchoolWarehouse.setItemNum(assetsSchoolWarehouse.getItemNum() + assetsOrders.getItemNum());
                assetsSchoolWarehouseMapper.updateAssetsSchoolWarehouse(assetsSchoolWarehouse);
            }


            assetsOrdersMapper.deleteAssetsOrdersById(id);
